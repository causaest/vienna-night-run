...
```{r}
summary(z$time_in_minutes)
```

```{r}
z %>% 
  ggplot(aes(x = time_in_minutes)) +
  geom_histogram(aes(y = ..density..), color="black", fill="gray", binwidth = 2) +
...
```{r}
z$company_bin <- ifelse(is.na(z$company), FALSE, TRUE)
table(z$company_bin)
```
...
```{r}
z %>% 
  ggplot(aes(x = company_bin, y = time_in_minutes)) +
  geom_boxplot() +
...
```{r}
t.test(time_in_minutes ~ company_bin, data = z)
```

```{r}
z %>% 
  ggplot(aes(x = time_in_minutes, fill = company_bin)) +
...
z %>% 
  filter(sex %in% c("m", "w")) %>% 
  ggplot(aes(x = factor(sex), y = time_in_minutes)) +
  geom_boxplot() +
  labs(title = "Effect of gender on time distribution", 
       x = "Gender (m: Male, w: Female)", y = "Time (minutes)")
```

```{r}
t.test(time_in_minutes ~ sex, data = filter(z, sex %in% c("m", "w")))
```

```{r}
zbin %>% 
  filter(sex %in% c("m", "w")) %>% 
  ggplot(aes(x = time_in_minutes, fill = sex)) +
  geom_density(alpha = 0.4) +
  scale_fill_discrete("Gender", labels = c("Male","Female")) +
...
```{r}
z %>% 
  ggplot(aes(x = factor(group), y = time_in_minutes)) +
  geom_boxplot() +
...
```{r}
zbin %>% 
  ggplot(aes(x = time_in_minutes, fill = group)) +
  geom_density(alpha = 0.4) +
  scale_fill_discrete(
    "Group", 
...
z %>% 
  group_by(group) %>% 
  summarise(min_time = min(time_in_minutes),
            mean_time = mean(time_in_minutes),
            quant = quantile(time_in_minutes, probs = 0.95),
            max_time = max(time_in_minutes))
```

```{r}
filter(z, group == 1) %>% 
  mutate(offset = time_in_minutes - 25) %>% 
  filter(offset > 0) %>% 
...
```{r}
filter(z, group == 1) %>% 
  mutate(offset = time_in_minutes - 28) %>% 
  filter(offset > 0) %>% 
  summarise(mean(offset), n())
```
...
```{r}
filter(z, group == 1) %>% 
  mutate(offset = time_in_minutes - 31) %>% 
  filter(offset > 0) %>% 
  summarise(mean(offset), n())
```
