...
read_vnr_raw <- function(year) {
  path_to_raw <- paste0("../data/raw/results-public-", year, ".txt")
...
```{r}
# # To check a particular year, run:
# 2025 |>
#     read_vnr_raw() |>
#     calc_time() |>
#     fix_factors() |>
...
write_vnr_full <- function(year) {
  results <- year |> 
    ...
    fix_factors() |> 
    correct_speed()
  path_to_full <- paste0("../data/full/results-full-", year)
...
write_vnr_clean <- function(year) {
  results <- year |> 
    ...
    correct_speed() |> 
    keep_cols()
  path_to_clean <- paste0("../data/clean/results-clean-", year)
